...
def GetVolumeFraction(psi0, psi1, psi2, EPS):
    ...
    if psi0 < -EPS and psi1 < -EPS and psi2 < -EPS: #CASE0
        return 1
    elif psi0 > EPS and psi1 > EPS and psi2 > EPS: #CASE0
        return 0
    
    ### if p0 is cut...
    if abs(psi0)<EPS and abs(psi1)>=EPS and abs(psi2)>=EPS: #psi0 is on vertex, psi1 and psi2 not
        if np.sign(psi1)!=np.sign(psi2):        # edge p1p2 is cut #CASE2
            s = 1 * (-psi1)/(psi2 - psi1)  #length ratio?? all triangles equilateral ?`
            if psi1 > 0:
                s = 1-s
        else:   #element is only cut in p0 (tangent) #CASE 1
            s = -0.5 * np.sign(psi1) + 0.5      # if psi1 <0 (-->psi2<0) then s=1, else (i.e. psi1, psi2>0) then s=0)
        return s
    elif abs(psi0)<EPS and abs(psi1)<EPS and abs(psi2)>=EPS: #edge p0p1 is on interface CASE3
        if psi2>0:
            s=0
        else:
            s=1
        return s
    elif abs(psi0)<EPS and abs(psi1)>=EPS and abs(psi2)<EPS: #edge p0p2 is on interface #CASE3 
        if psi1>0:
            s=0
        else:
            s=1
    ...
    if abs(psi0)>=EPS and abs(psi1)>=EPS and abs(psi2)>=EPS:   #CASE4 
        if np.sign(psi1) == np.sign(psi2) and np.sign(psi0) != np.sign(psi1): #lines 01 and 02 are to be cut !
            s = psi0*psi0 / ((psi1 - psi0)*(psi2 - psi0)) # why is it the product !
            if psi0 > 0:
                s = 1-s
...
def InterpolateLevelSetToElems(levelset_p1, val1, val2, func_p0, mesh, EPS ):
    ...
    for i,el in enumerate(mesh.ngmesh.Elements2D()):
        ...
        psi0_2[1] = psi1
        psi0_2[2] = psi2
        positive_count = np.sum(psi0_2> EPS)
        negative_count = np.sum(psi0_2 < -EPS)
        zero_count = np.sum(abs(psi0_2) <EPS)
        #print("Element Number:",i)
        #print("[#+ , #-, #0]",positive_count,negative_count,zero_count)
        #print(el.vertices,"when indexing -1")            #Debug
        #print("psi0_2",psi0_2)
